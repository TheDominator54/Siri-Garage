...
var options = {
  port: 1883,
  host: '192.168.1.190',
  clientId: 'DomPi_LittleGarage'
};
var client = mqtt.connect(options);
console.log("Little Garage Connected to MQTT broker");
...
var GARAGE_DOOR = {
  opened: false,
  open: function() { 
    console.log("Opening the Little Garage!");
    client.publish('inTopic2', '0');
  },
  close: function() { 
    console.log("Closing the Little Garage!");
    client.publish('inTopic2', '1');
  },
  identify: function() {
    console.log("Identify the Little Garage");
    // nothing to do.
  }
};
console.log("Little Garage door set as %s",GARAGE_DOOR.opened);


// Add properties for publishing (in case we're using Core.js and not BridgedCore.js)
garage.username = "C1:5D:3F:EE:5E:FC";
...
garage
  ...
  .on('get', function(callback) {
    ...
    if (GARAGE_DOOR.opened) {
      console.log("Query: Is Little Garage Open? Yes.");
      callback(err, Characteristic.CurrentDoorState.OPEN);
    }
    else {
      console.log("Query: Is Little Garage Open? No.");
